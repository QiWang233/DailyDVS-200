...
# class OurDVS(sjds.NeuromorphicDatasetFolder):
#     def __init__(
#             self,
#             root: str,
...
#             extract_root2 = '/home/hanbing/data'
#             with open(os.path.join(extract_root, f'{prefix}_label.txt')) as txt_file:
#                 for line in txt_file:
#                     if line.__len__() > 1:
#                         # e.g., "action_001/dvSave-2021_10_15_19_18_02"
#                         class_name, sample_name, _ = line.split(' ')[0].split('/')
#                         source_file = os.path.join(extract_root2, 'events_np', class_name,sample_name ,sample_name + '.npz')
...
class OurDVS(sjds.NeuromorphicDatasetFolder):
    ...
    def create_events_np_files(extract_root: str, events_np_root: str):
        for prefix in ('train', 'val', 'test'):
            ...
            extract_root2 = '/home/hanbing/data'
            txt_path = 'id_list'
            with open(os.path.join(extract_root,txt_path ,f'{prefix}_label.txt')) as txt_file:
                for line in txt_file:
                    if line.__len__() > 1:
                        # e.g., "action_001/dvSave-2021_10_15_19_18_02"
                        class_name, sample_name, _ = line.split(' ')[0].split('/')
                        source_file = os.path.join(extract_root2, 'events_np2', class_name,sample_name ,sample_name + '.npy')
                        # if os.path.exists(source_file):
                        target_file = os.path.join(target_dir, class_name, sample_name + '.npy')
                        os.symlink(source_file, target_file)
