...
from utils.utils import  save_checkpoint, NativeScalerWithGradNormCount, save_best_checkpoint, \
    load_checkpoint, load_pretrained, auto_resume_helper, reduce_tensor
...
def set_e2s(config):
    if config.DATA.DATASET == 'our_dvs':
        e2s = E2SRC([320, 320], batch_size=config.DATA.BATCH_SIZE)
    else:
...
def main(config):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if ((epoch % 5) == 0 and epoch <= 300) or epoch >= 300:
        # if True:
            acc1, acc5, loss = validate(config, e2s, data_loader_val, model)
            logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
