...
class Group_Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.dim() != 4:
            ...
            # C = int(x.numel() // (x.shape[0] * hw * hw))
            # x = x.reshape(x.shape[0], 80, 60, -1)
            # print(x.shape)
...
class GET(nn.Module):
    ...
    def __init__(self,
        ...
        # embed_split = 24
        input_dim = 2 * embed_split * int(patch_size ** 2) # 768
        hidden_dim = int(group_num * 64 / (group_num / 12)) # 768
    ...
    def forward(self, x):
        """Forward function."""
        # print('******************************************************************')
        # print(f'get x.shape start:{x.shape}')
        x = self.channel_embed(x)
        # print(f'after gte x.shape:{x.shape}')
        ...
        for i in range(self.num_layers):
            ...
            # print(f'get layer {i} start:{x.shape} Wh:{Wh} Ww:{Ww}')
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww)
            # print(f'get layer {i} after:{x.shape} Wh:{Wh} Ww:{Ww} xout:{x_out.shape}, h:{H}, w:{W}')
        ...
        # print(f'get x.shape final:{x.shape}')
        # print('******************************************************************')
        return outs
