...
class C3D(nn.Module):
    ...
    def forward(self, x):
        ...
        #x.shape torch.Size([30, 3, 16, 112, 112])
        x = self.conv1a(x)#torch.Size([30, 64, 16, 112, 112])
        x = self.pool1(x)#torch.Size([30, 64, 16, 56, 56])

        x = self.conv2a(x)#torch.Size([30, 128, 16, 56, 56])
        x = self.pool2(x)
        ...
        x = self.conv5a(x)
        x = self.conv5b(x)
        x = self.pool5(x) #torch.Size([30, 512, 1, 4, 4])

        x = x.flatten(start_dim=1)#torch.Size([30, 8192])
        x = self.relu(self.fc6(x))#torch.Size([30, 4096])
        x = self.dropout(x)#torch.Size([30, 4096])
        x = self.relu(self.fc7(x))#torch.Size([30, 4096])
