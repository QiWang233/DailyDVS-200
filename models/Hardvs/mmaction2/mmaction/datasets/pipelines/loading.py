...
class SampleFrames:
    ...
    def _get_train_clips(self, num_frames):
        ...
        # num_frames = 80 ori_clip_len = 40  41/2 = 20
        # 一个片段要划分的总帧数
        ori_clip_len = self.clip_len * self.frame_interval
        # default keep_tail_frames=False
        if self.keep_tail_frames:
            avg_interval = (num_frames - ori_clip_len + 1) / float(
                self.num_clips)
            # 总帧数比一个片段的总帧数多
            if num_frames > ori_clip_len - 1:
                # np.arange 生成从0开始，步长为1的等差数列，个数为num_clips
                base_offsets = np.arange(self.num_clips) * avg_interval
                # 生成[0,avg_interval]的随机偏移量
                # np.random.uniform生成num_clips个[0,avg_interval]均匀分布的随机数
            ...
            if avg_interval > 0:
                ...
                clip_offsets = np.zeros((self.num_clips, ), dtype=np.int64)
        # print("base_offsets = {}".format(base_offsets) + "clip_offsets = {}".format(clip_offsets))
        return clip_offsets
    ...
    def __call__(self, results):
        ...
        # clip_offset 表示每个片段的偏移量
        # clip_len 每个片段要截取的帧数
        # frame_interval 帧间隔
        # clip_offsets[:, None] 将clip_offsets转成列向量
        # np.arange 生成一个[0,clip_len-1]的行向量
        # offset = clip_offsets[:, None]
        # print("offset = ",offset)
        ...
        # temporal_jitter=FLASE
        if self.temporal_jitter:
            perframe_offsets = np.random.randint(
                self.frame_interval, size=len(frame_inds))
            frame_inds += perframe_offsets
        # 根据片段数num_clips 将其重组为二维数组
        frame_inds = frame_inds.reshape((-1, self.clip_len))
        # print("frame_inds = {}".format(frame_inds) + "total_frames = {}".format(total_frames))
        # 默认loop
        if self.out_of_bound_opt == 'loop':
            # 取余 [0,5,10,15,20,25,30,35] % total_frames
...
class RawFrameDecode:
    ...
    def __call__(self, results):
        ...
        # results['frame_inds'] = results['frame_inds']*5
        if results['frame_inds'].ndim != 1:
            results['frame_inds'] = np.squeeze(results['frame_inds'])
