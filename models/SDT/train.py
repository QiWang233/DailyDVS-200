...
from spikingjelly.datasets.our_dvs import OurDVS
# from ourdvs import OurDVS
import torch
...
def main():
    ...
    if args.dataset == "cifar10-dvs-tet":
    ...
    elif args.dataset == "our_dvs":
        dataset_train = OurDVS(
            args.data_dir,
            train=True,
            data_type="frame",
            frames_number=args.time_steps,
        ...
        # dataset_train = OurDVS(root=args.data_dir, data_type='frame', frames_number=args.time_steps, split_by='number', train=True)
        # dataset_eval = OurDVS(root=args.data_dir, data_type='frame', frames_number=args.time_steps, split_by='number', train=False)
    else:
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if not args.prefetcher or args.dataset in dvs_utils.DVS_DATASET:
            ...
            # print(input.shape, target.shape, input, target)
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=""):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            functional.reset_net(model)
            # pdb.set_trace()
            # acc1, acc5 = accuracy(pred_list,target_list,output, target,topk=(1, 5))
    ...
    # classes_results = top_k_classes(pred_list,target_list)
    # txt_path = "/home/wangqi/Spike-Driven-Transformer-main/id_test.txt"
    # total_acc = 0
    # for i in classes_results:
    #     total_acc += i[1]
    # print(total_acc / len(classes_results))
    # with open(txt_path,'a') as file:
    #     file.write(f"{args.id} {total_acc / len(classes_results)}\n")
    metrics = OrderedDict(
